...
The $\alpha_{k}$ represents a mixing weight for the kth component and $\sum_{k=1}^{K} = 1$. The $f_k(x)$ components in principle are arbitrary in the sense that you can choose any sort of distribution. In practice, parametric distributions, like gaussians, are often used since a lot work has been done to understand their behaviour. So when you substitute each $f_k(x)$ for a gaussian you get gaussian mixture models (GMM). Likewise, if you substitute each $f_k(x)$ for a binomial distribution, you get binomial mixture models (BMM). Since each parametric distribution has it's own parameters, we can represent the parameters of each component with a $\theta_{k}$:

$$f(x) = \sum_{k=1}^{K}\alpha_{k}f_{k}(x;\theta_{k})$$

## Why Would You Use a Mixture Model?

A Mixture model is probabilistic model for representing subpopulations of data without requiring that the observed data to be first labeled to identify the subpopulations. Mathematically, it is

Let's use an example to demonstrate this more concretely

To explain what mixture models are, lLet's say someone presented you with the following density plot: 
