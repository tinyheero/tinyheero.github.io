...
$$P(X) = \sum_{k=1}^{K}\alpha_k\mathcal{N}(X|\mu_{k},\sigma_{k}^{2})$$ 
...
```{r}
comp1.prod <- dnorm(mixmdl$x, mixmdl$mu[1], mixmdl$sigma[1]) * 
	mixmdl$lambda[1]

comp2.prod <- dnorm(mixmdl$x, mixmdl$mu[2], mixmdl$sigma[2]) * 
	mixmdl$lambda[2]
