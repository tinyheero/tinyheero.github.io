...
First, we have to undestand The way to think about this problem is to understand that this is a parameter estimation (i.e. model selection) problem. We will first start this post by discussing a bit about parameter estimation. Then we will introduce the expectation-maximization (EM) algorithm and how it can be used in parameter estimation. Then we will walk through how to implement the EM algorithm. 
...
```{r parameter_est_complete_data, dev = "svg", message = FALSE, fig.height = 2}
library("ggplot2")
library("dplyr")
library("reshape2")
...
```{r parameter_est_incomplete_data, dev = "svg", message = FALSE, fig.height = 2}
vals.df %>%
...
```{r kmeans_init, dev = "svg", fig.height = 4}
