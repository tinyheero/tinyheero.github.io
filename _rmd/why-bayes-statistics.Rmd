...
# It's all about the uncertainity, about the uncertainity, no certainity!
...
uncertainity about an unknown quantity. To illustrate this, imagine I had the 
...
**The point is there is some uncertainity in your estimation of this unknown 
quantity.**
...
have no uncertainity in their value; They are either that value or they are not. 
...
We have learned that that it's all about the uncertainity in our estimations.
...
# Where do these probability distributions for representing uncertainity come from?
...
heads or tails. Beta(1, 1) represents a global uncertainity in that the 
...
# So what do I do with this uncertainity probability distribution?
...
the binomial distribution (this mathemetically forms what is called a 
beta-binomial) and then we can generate some expected outcomes. As you might 
guess, the shape of your beta will influence your expected outcomes. For 
instance, here we test the different effects of the 6 beta distributions on
the distribution of the expected number of heads if we were to flip a coin 20 
times:

```{r implications-of-a-prior, fig.width = 12, fig.height = 10}
plot_prior_prediction_distr <- function(in_data) {
  ...
  predictive_p <- 
    coin_flip_df %>%
    ggplot(aes(x = factor(heads, levels = 0:num_trials))) +
    geom_bar() +
...
uncertainity in your <span class="inlinecode">$\theta$</span> parameter for your
...
representing uncertainity regarding an unknown quantity. This uncertainity is
...
* "[Introduction to Bayesian statistics using BUGS" (offered by the MRC Biostatisics Unit from the University of Cambridge)](https://www.mrc-bsu.cam.ac.uk/training/short-courses/bayescourse/)
