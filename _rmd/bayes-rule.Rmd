...
Say we are playing cards and we have a prior probability that there is a <span class="inlinecode">$\frac{1}{13}$</span> chance of getting a King card. How do we know this? Well our prior knowledge tells us that there are 52 cards in a deck with a total of 4 suits. Each suit has its own Jack, Queen, King (i.e. face card), and so there must be 4 Kings in total. Therefore, <span class="inlinecode">$\frac{4}{52} = \frac{1}{13}$</span>. 

But now say, we also have the data that the card we have in hand is a "face" card (i.e. Jack, Queen, King). What then is the probability of getting a King now? What we need to solve is the posterior, <span class="inlinecode">$P(H\ |\ E)$</span>, and we use Baye's Rule to help us solve this. Let's break down the individual components of Bayes' rule here:
...
### Example of Bayes' Rule Using Disease Testing
