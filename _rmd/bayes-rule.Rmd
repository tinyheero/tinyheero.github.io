...
This is defined as the Bayes' rule. Notice how this is exacty the same as the conditional probability with the only difference being that the joint probability <span class="inlinecode">$P(X, Y)$</span> has been expanded out to be <span class="inlinecode">$P(Y\ |\ X)\ P(X)$</span>. The rule doesn't seem to be that interestingly at first glance, but we will see below with some examples why it is actually really important.
...
<tr>
<th><span class="inlinecode">$Y_{1}$</span></th>
<th>.</th>
<th><span class="inlinecode">$Y_{i}$</span></th>
<th>.</th>
</tr>
</thead>
  <tr>
