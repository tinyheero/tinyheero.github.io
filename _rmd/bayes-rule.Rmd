...
In my previous post on [Joint, Marginal, and Conditional Probabilities post]({% post_url 2016-03-20-basic-prob %}), I introduced 3 different types of probabilities. One famous probability rule that is built on these probabilities is called "Bayes' Rule" which forms the basis of bayesian statistics. 
...
## Bayes' Rule Example #1: Playing Cards
...
As another example, we will use disease testing to illustrate bayes' rule. Say we know that in the general populance, the chances that a person will have a disease is 0.001. We also know that the test has a true positive rate of 99% chance and a false positive rate of 5%. If a person gets the test and it comes back as positive, what are the chances that the person actually has the disease. Let's first setup our variables here:

* <span class="inlinecode">$P(H = disease)$</span>: This is our prior belief of the probability of having the disease. This is 0.001.
* <span class="inlinecode">$P(T = positive | H = disease)$</span>: This is the probability of the test being positive given the person has the disease. In other words, the true positive rate which is 0.99.
* <span class="inlinecode">$P(T = positive)$</span>: This is the probability of the test being positive irrespective of whether the person has the disease or not. This would be 0.99 * 0.001 + 0.05 * 0.999.

* <span class="inlinecode">$P(H = disease | T = positive)$</span>: This is the thing (posterior probability) we are most interested in knowing. What is the probability of actually having the disease if our test result is positive? 
...
(0.9 * 5/100) / (0.9*5/100 + 0.1*95/100)
