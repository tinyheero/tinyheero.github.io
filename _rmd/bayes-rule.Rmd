...
For the rest of this post, I will be using variables, H and E to represent the Bayes' rule. Now, let's see this rule in action.
...
* <span class="inlinecode">$P(H = King) = \frac{1}{13}$</span>.
* <span class="inlinecode">$P(E = Face\ card)$</span> = As there are 3 face cards per suit and there are 4 suits, then this is <span class="inlinecode">$\frac{12}{52} = \frac{3}{13}$</span>
* <span class="inlinecode">$P(E = Face\ card\ |\ H = King)$</span> = Since every single King card is a face card, this probability has to be 1.

Now we just fill this in into the Bayes' Equation:

<div>
$$\begin{align}
P(H = King\ |\ E = Face\ card) &= \frac{1 * \frac{1}{13}}{\frac{3}{13}} \\
P(H = King\ |\ E = Face\ card) &= \frac{1}{3}
\end{align}$$
</div>

Basically we started with a prior belief, <span class="inlinecode">$P(H = King) = \frac{1}{13}$</span>, but with new information regarding the card being a face card we are more confident that the card is a King now, <span class="inlinecode">$P(H = King\ |\ E = Face\ card) = \frac{1}{3}$</span>.
...
* <span class="inlinecode">$P(H = disease)$</span>: This is our prior belief on the probability of having the disease. This is 0.001.
* <span class="inlinecode">$P(E = positive\ |\ H = disease)$</span>: This is the probability of the test being positive given the person has the disease. In other words, the true positive rate which is 0.95.
* <span class="inlinecode">$P(E = positive)$</span>: This is the probability of the test being positive irrespective of whether the person has the disease or not. So this would include all true positive events and false positive events. So it it would be summation of the two joint probabilities: 

    <div>
    $$\begin{align}
    P(E = Positive) &= P(E = Positive, H = Disease) + P(E = Positive, H \neq Disease) \\
    P(E = Positive) &= 0.95 * 0.001 + 0.01 * (1-0.01) \\
    P(E = Positive) &= 0.01094
    \end{align}$$
    </div>
...
<div>
$$\begin{align}
P(H = Disease\ |\ E = Positive) &= \frac{P(E = positive\ |\ H = disease)\ P(H = disease)}{P(E = positive)} \\
P(H = Disease\ |\ E = Positive) &= \frac{0.95 * 0.001}{0.01094} \\
P(H = Disease\ |\ E = Positive) &= 0.087
\end{align}$$
</div>
...
So far, we've defined the the prior probability, <span class="inlinecode">$P(H)$</span>, and posterior probability, <span class="inlinecode">$P(H\ |\ E)$</span>, but the other components of the Bayes' rule have names themselves that you should be aware of. 

* <span class="inlinecode">$P(E\ |\ H)$</span>: Likelihood. This is the probability of the data being generated given the hypothesis.
* <span class="inlinecode">$P(E)$</span>: Marginal likelihood or evidence. This is just the overall probability of the data after we marginalize out all the possible hypothesis.
