...
So how do we actually figure out what the pmf/pdf is of a random variable? Thankfully, we don't actually need to figure this out. A lot of brillant statisticians have done the leg work for us in deriving pmf/pdf that we can readily use (e.g. binomial/gaussian). You may have heard the phrase that random variable "Y follows a gaussian distribution". What this means is that they are assuming the data being generated comes from a normal distributions. Making these assumptions allows us to leverage off the known properties of well-established probability distributions. For instance, we know that the pdf of a gaussian is:

<div>
$$f(Y \,|\, \mu, \sigma^{2}) = \frac{1}{\sigma\sqrt{2\pi}}e^{-\frac{(x - \mu)^{2}}{2\sigma^{2}}}$$
</div>

This is parameterized by a mean <span class="inlinecode">$\mu$</span> and a standard deviation <span class="inlinecode">$\sigma$</span>. A standard gaussian (<span class="inlinecode">$\mu = 0, \sigma = 1$</span>) would look like this. 

```{r std_normal_distr, fig.height = 5}
set.seed(1)
...
```{r std_normal_distr_integral, fig.height = 5}
# dnorm provides the probability density
data.frame(x = vals) %>%
  ggplot(aes(x = vals)) +
  stat_function(fun = dnorm) +
...
So basically by making assumptions that our random variables follow an established probability distribution, we can make use of already known pmf/pdf and apply the same principles that we learned above to ask questions we have about the data. One caveat of this approach is that our assumption could be wrong. **This means that one needs to be careful when deciding on a particular probability distribution**.

Thankfully there are some well established principles already to help us. The figure below provides a decision tree that gives you an idea of what are some common probability distributions that one can use given the data they have in hand:
...
* There are already established probability distributions with pmf/pdf (e.g. binomial/gaussian) that we can use in our work.
