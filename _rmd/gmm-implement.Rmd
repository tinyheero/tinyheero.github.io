...
First, we have to undestand The way to think about this problem is to understand that this is a parameter estimation (i.e. model selection) problem. We will first start this post by discussing a bit about parameter estimation and specifically in the context of two scenarios: Complete and Incomplete Data.
...
Now that we have the initial parameters of our GMM, we now have to determine what is the probability (soft label; responsibility) that the data point (x~j~) belongs to component (k~i~)? This is considered the expectation step (E-step) of MLE where we are calculating the "expectation values" of the soft labels for each data point. 
...
data.frame(x = wait) %>%
  ...
  stat_function(geom = "line", fun = plot_mix_comps,
                arg = list(m.step$mu[1], sqrt(m.step$var[1]), lam = m.step$alpha[1]),
                colour = "red", lwd = 1.5) +
  stat_function(geom = "line", fun = plot_mix_comps,
                arg = list(m.step$mu[2], sqrt(m.step$var[2]), lam = m.step$alpha[2]),
                colour = "blue", lwd = 1.5) +
  ylab("Density")
```
...
In this post, I have demonstrated how to implement your own mixture model where each component is a gaussian. The logic can easily be extended to other types of mixture models by simply substituting the components for appropriate statistical distribution needed. We used the well known expectation maximization algorithm to allow us to use maximum likelihood estimation in the incomplete data scenario. 
