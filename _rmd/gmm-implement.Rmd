...
In my previous post ["Using Mixture Models for Clustering in R"]({% post_url 2015-10-13-mixture-model %}), I covered the concept of mixture models and how one could use a gaussian mixture model (GMM), one type of mixure model, for clustering. If you are like me, not knowing what is happening "under the hood" may bug you. What is actually happening when I run the `normalmixEM` function?  How does it know where to "put" the components? 

The way to think about this problem is to understand that this is a parameter estimation (i.e. model selection) problem. This will be becore more clear below:

```{r}
#Where are these posterior probabilites coming from?
```
...
Let's say you believe the data source is actually a gaussian distribution. Then with these values and knowledge of their data source, how could you derive the parameters of the gaussian distribution that gave this data? 

Remembering that the gaussian distribution is parameterized by two variables:
...
As it turns out, what've just done is called maximum likelihood estimation (MLE). In layman terms, what we've essentially done is estimate the parameters that most likely gave rise to the data we are observing. 
...
# 	geom_point() +
# 	stat_function(fun = dnorm, args = list(comp1.vals, 0.5, 1))
```

Without the labels on the data indicating their source, we are hopeless in applying MLE. But if we somehow find a way to "complete" (i.e. find the labels for each data point), then we can go back to using maximum likelihood estimation. One way around this could be to:
...
This iterative approach is the basis for something called the expectation maximization (EM) algorithm. The main difference is rather than assigning the data to one of the possible gaussians, EM considers the possibilities of the data belonging to all of the possible gaussians. This is analogous to the concept of "hard" and "soft" labeling that I mentioned in my previous post ["Using Mixture Models for Clustering in R"]({% post_url 2015-10-13-mixture-model %}). Working in a probablistic framework allows us to assign probabilities (weights) of data belonging to a gaussian so that we don't have to "commit" the data to any one gaussian. 
...
Now that we have these concepts in play, let's get down to actually figuring out how to fit our own GMM!

## Mathematical Representation of a GMM

If we want to fit our own GMM, we have to understand how to mathematically (it's not that bad) a GMM:
...
For GMM, it's very common to first run k-means on your data to get some hard-labels on the data. With these hard-labels, we can use MLE to estimate the component parameters for our initialization (remember MLE works in the "Complete Data" scenario). Let's try that here:
...
## Calculating the Component Responsibilities of Each Data

Now that we have the initial 
Posterior Probabilities (i.e. Responsibilities)
